    ...
    <div class="bg-white p-8 rounded-lg shadow-md w-full max-w-md">
      ...
      <form @submit.prevent="submit">
        ...
        <div class="mb-4">
          ...
          <input v-model="form.stock_quantity" type="number" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" required />
          <div v-if="form.errors.stock_quantity" v-text="form.errors.stock_quantity" class="text-red-500 text-xs mt-1"></div>
        </div>
        <button type="submit" class="w-full bg-blue-500 text-white py-2 rounded-lg hover:bg-blue-600" :disabled="form.processing">Update Product</button>
      </form>
    </div>
  </div>
</template>
